...
- `PresenceMonitor` (shipped, Phase 8/D76) publishes this router's compact `RouterHealth`
  ...
  observable from any single WAN point (the C2 node map). Per-link quality (loss, RTT,
  congestion) is the `LinkStatsCollector`'s domain, not presence — see
  [mesh-presence-approaches.md](mesh-presence-approaches.md) for the rationale. The
