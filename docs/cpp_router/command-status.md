...
| ~~`SET_PARTICIPANT_PARTITION`~~ **Retired (D73)** — replaced by `ADD_PARTICIPANT_PARTITION`/`REMOVE_PARTICIPANT_PARTITION` | Multi-valued, incremental change to the participant-level partition on a named participant (D83) — targets `platform_wan`/`control_wan` post-D103, was `team_wan` before its retirement | live-apply via participant `set_qos` (`RouteEntityFactory::apply_participant_partition`), update `RouterParticipantStatus.participant_partition`; the protected node-identity entry (and, on `control_wan`, its standing wildcard, D103) is never removable |
