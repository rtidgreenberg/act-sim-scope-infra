...
- [SPDP2/Boot-Init Tasks](spdp2-and-boot-init-tasks.md): two sequential, self-contained task assignments (hand off to a fresh session) — (1) root-cause D87's SPDP2 delayed-enable failure and decide go/no-go on reviving it; (2) a wildcard-partition-before-any-routes boot pattern for full-mesh roster collection without leaking team-topic SEDP metadata, timing-dependent on task 1's outcome.
- [WAN DP Consolidation Task](wan-dp-consolidation-task.md): self-contained task assignment — does matching via a per-platform unique partition ID (instead of a shared team name) change the SEDP-metadata-exposure math that killed D84's platform_wan/team_wan merge (expected: no); and does extending `RouterHealth` to also report `team_wan`'s presence (today untracked entirely — a real, previously undocumented gap) get the same diagnostic simplicity without the exposure risk.
- [`is_wan` Flag Decomposition Task](is-wan-flag-decomposition-task.md): self-contained task assignment — the `is_wan` participant flag fuses two unrelated concepts (D83 team-partition scoping = team_wan only, and link-stats WAN-leg detection = platform_wan **and** team_wan) under a name that misleads (all `*_wan` participants are on the WAN). Suspected consequence, verify first: since D87 unset `platform_wan`'s flag, its data-route legs may no longer be link-stats-covered. Split into two accurately-named flags. Surfaced by D93 (mesh dashboard), which sidestepped the flag rather than touch it.
