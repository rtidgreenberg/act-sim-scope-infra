// Sha256.cxx — FIPS 180-4 SHA-256 (see Sha256.hpp for why this is hand-rolled).
...
void compress(std::uint32_t h[8], const unsigned char block[64]) {
    std::uint32_t w[64];
    for (int i = 0; i < 16; ++i) {
...
std::string sha256_hex(const void *data, std::size_t len) {
    ...
    tail[remaining] = 0x80;
    const std::size_t tail_len = (remaining < 56) ? 64 : 128;
    ...
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 4; ++j) {
            unsigned char byte = static_cast<unsigned char>(h[i] >> (8 * (3 - j)));
