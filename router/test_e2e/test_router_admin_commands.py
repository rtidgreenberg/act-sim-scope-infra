...
Asserts the Phase 6 6a evidence items (D56):
  ...
  E2    ENABLE_ROUTE -> ROUTE_WAITING_FOR_DISCOVERY (no source writer) then ROUTE_ENABLED
        (source writer present); ack accepted; state_revision bumps.
...
def test_admin_command_control_loop(
    ...
    try:
        ...
        # (E2a) ENABLE_ROUTE with no source writer yet -> WAITING; ack accepted; revision up.
        ...
        assert waiting is not None and waiting["state"] == "ROUTE_WAITING_FOR_DISCOVERY", \
            f"route never reached ROUTE_WAITING_FOR_DISCOVERY; got {waiting}; " \
            f"log {router.log_path}"
        ...
        # (E2b) a source writer appears -> the route builds -> ENABLED. Keep the reference
        # alive (a dropped DataWriter is GC'd and the endpoint lost immediately).
        ...
        assert enabled is not None and enabled["state"] == "ROUTE_ENABLED", \
            f"route never reached ROUTE_ENABLED after source writer appeared; " \
            f"got {enabled}; log {router.log_path}"
